...
export class PongGame {
  ...
  processInput = () => {
	if (
	  ...
	  const lastPosition = {
		x: Math.round(this.states.paddle.players[this.playerNumber].position.x * 1000) / 1000,
		y: Math.round(this.states.paddle.players[this.playerNumber].position.y * 1000) / 1000
	  };
  
	  // 의미있는 위치 변화가 있는 경우만 업데이트
	  if (Math.abs(currentX - lastPosition.x) >= 0.001 || Math.abs(currentY - lastPosition.y) >= 0.001) {
			...
			updateGameState({ ball, score }) {
			  ...
			  updateGameObjects() {
				...
				if (this.playerNumber === 'player1') {
					// Player 1 시점
					this.objects.playerPaddle.position.lerp(player1Position, LERP_FACTOR.paddle);
					this.objects.opponentPaddle.position.lerp(player2Position, LERP_FACTOR.paddle);
				} else {
					// Player 2 시점: 위치 반전
					...
					this.objects.playerPaddle.position.lerp(invertedPlayer2Pos, LERP_FACTOR.paddle);
					this.objects.opponentPaddle.position.lerp(invertedPlayer1Pos, LERP_FACTOR.paddle);
				}
			 }
